...
async function registerPassword(req, res, next) {
  try {
    ...
    if (!displayName || !username || !password || !backupEmail) {
      return res
        .status(400)
        .json({ message: 'displayName, username, password, and backupEmail are required' })
    }
    ...
    const member = await authService.registerPassword({
      displayName,
      username,
      password,
      backupEmail,
...
async function startPasskeyRegistration(req, res, next) {
  try {
    ...
    if (!displayName || !username || !backupEmail) {
      return res
        .status(400)
        .json({ message: 'displayName, username, and backupEmail are required' })
    }
...
async function finishPasskeyLogin(req, res, next) {
  try {
    ...
    const { accessToken, idToken, refreshToken } = await passkeyService.finishAuthentication({
      sessionId,
      credential,
    })
...
async function registerWithTelegram(req, res, next) {
  try {
    ...
    if (!displayName || !username || !telegramData) {
      return res
        .status(400)
        .json({ message: 'displayName, username, and telegramData are required' })
    }
    ...
    const member = await telegramService.registerWithTelegram({
      telegramData,
      displayName,
      username,
    })
...
async function loginWithTelegram(req, res, next) {
  try {
    ...
    const { accessToken, idToken, refreshToken } = await telegramService.loginWithTelegram({
      telegramData,
    })
...
// OIDC UserInfo endpoint — returns the bearer's claims for the full first-party
// scope set (profile, email, address, phone, membership).
async function userinfo(req, res, next) {
  try {
    const [member, profile] = await Promise.all([
      memberService.findById(req.memberId),
      profileService.findByMemberId(req.memberId),
    ])
    return res.json(oidcService.getClaims(member, profile, oidcService.FIRST_PARTY_SCOPES))
  } catch (err) {
    next(err)
  }
}
