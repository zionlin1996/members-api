...
const PENDING = {
  id: SESSION_ID,
  challenge: CHALLENGE,
  context: { displayName: 'Test User', username: 'testuser', backupEmail: 'backup@example.com', type: 'registration' },
...
describe('POST /auth/login/passkey/finish', () => {
  ...
  it('200 — returns accessToken and sets cookie', async () => {
    ...
    const res = await request(app).post('/auth/login/passkey/finish').send({
      sessionId: SESSION_ID,
